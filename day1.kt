package com.victormrequena.day1

fun main() {
    val input = """2xjzgsjzfhzhm1
...
9lgmxktj1frxl"""
    ...
    val result2 = lines.map {
        val matches = regex.findAll(it)
        val first = matches.first().groups[1]?.value
        val last = matches.last().groups[1]?.value
        (first?.toNumber() + last?.toNumber()).toInt()
    }
        .reduce { e1, e2 -> e1 + e2 }
