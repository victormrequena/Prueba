package com.victormrequena.day2
...
fun main() {
    val input = """Game 1: 10 red, 7 green, 3 blue; 5 blue, 3 red, 10 green; 4 blue, 14 green, 7 red; 1 red, 11 green; 6 blue, 17 green, 15 red; 18 green, 7 red, 5 blue
...
Game 100: 16 red, 3 blue; 2 red, 5 green; 9 red; 1 blue, 3 green, 10 red; 1 red, 5 blue, 3 green; 12 blue, 9 red"""
    ...
    val colorsRegex = Regex("\\d+(?= (red|blue|green))")

    val lines = input.lines()
    val result1 = lines.sumOf { line ->
        val game = gameRegex.find(line)!!
        val colors = colorsRegex.findAll(line)

        val validGame = colors
            .groupBy({ it.groupValues[1] }, { it.groupValues[0].toInt() })
            .all { entry ->
                when (entry.key) {
                    "red" -> entry.value.all { it <= 12 }
                    "green" -> entry.value.all { it <= 13 }
                    "blue" -> entry.value.all { it <= 14 }
                    else -> false
                }
            }
        if (validGame)
