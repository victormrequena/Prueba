package com.victormrequena.day5
...
fun main() {
    val input = """seeds: 41218238 421491713 1255413673 350530906 944138913 251104806 481818804 233571979 2906248740 266447632 3454130719 50644329 1920342932 127779721 2109326496 538709762 3579244700 267233350 4173137165 60179884
...
1542227407 1156260177 112739686"""
    val seeds = "seeds:"
    val seedToSoil = "seed-to-soil map:"
    val soilToFertilizer = "soil-to-fertilizer map:"
    val fertilizerToWater = "fertilizer-to-water map:"
    val waterToLight = "water-to-light map:"
    val lightToTemperature = "light-to-temperature map:"
    val temperatureToHumidity = "temperature-to-humidity map:"
    val humidityToLocation = "humidity-to-location map:"

    val numberRegex = Regex("\\d+")
    val seedList = input.lines().takeWhile { it.startsWith(seeds) }
        .flatMap {
            numberRegex.findAll(it)
        }
        .map { it.value.toLong() }

    val coordinatesRegex = Regex("(\\d+)\\s+(\\d+)\\s+(\\d+)")
    val seedToSoilMap = input.lines().dropWhile { !it.startsWith(seedToSoil) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }

    val soilToFertilizerMap = input.lines().dropWhile { !it.startsWith(soilToFertilizer) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    val fertilizerToWaterMap = input.lines().dropWhile { !it.startsWith(fertilizerToWater) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    val waterToLightMap = input.lines().dropWhile { !it.startsWith(waterToLight) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    val lightToTemperatureMap = input.lines().dropWhile { !it.startsWith(lightToTemperature) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    val temperatureToHumidityMap = input.lines().dropWhile { !it.startsWith(temperatureToHumidity) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    val humidityToLocationMap = input.lines().dropWhile { !it.startsWith(humidityToLocation) }.drop(1).takeWhile { it.isNotEmpty() }
        .map {
            val (destination, source, range) = coordinatesRegex.find(it)!!.destructured
            Coordinates(destination.toLong(), source.toLong(), range.toLong())
        }
    ...
    val result = seedList
        .asSequence()
        .map { seed ->
            val coordinate = seedToSoilMap.firstOrNull { seed in it.source until it.source + it.range }
            val value = if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
            value
        }
        .map { seed ->
            val coordinate = soilToFertilizerMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
        ...
        .map { seed ->
            val coordinate = fertilizerToWaterMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
        ...
        .map { seed ->
            val coordinate = waterToLightMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
        ...
        .map { seed ->
            val coordinate = lightToTemperatureMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
        ...
        .map { seed ->
            val coordinate = temperatureToHumidityMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
        ...
        .minOf { seed ->
            val coordinate = humidityToLocationMap.firstOrNull { seed in it.source until it.source + it.range }
            if (coordinate != null)
                calculateDestination(seed, coordinate.destination, coordinate.source, coordinate.range)
            else
                seed
    ...
    val rangesRegex = Regex("(\\d+)\\s+(\\d+)")
    val seedRangeList = input.lines().takeWhile { it.startsWith(seeds) }
        .flatMap { line ->
            rangesRegex.findAll(line)
                .map {
                    val (source, range) = it.destructured
                    (source.toLong() until source.toLong() + range.toLong())
                }
        }
    ...
    val result2 = (0..Long.MAX_VALUE)
        .first { location ->
            val humidity = humidityToLocationMap.firstOrNull { location in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(location, it.source, it.destination, it.range)
                } ?: location
            val temperature = temperatureToHumidityMap.firstOrNull { humidity in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(humidity, it.source, it.destination, it.range)
                } ?: humidity
            val light = lightToTemperatureMap.firstOrNull { temperature in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(temperature, it.source, it.destination, it.range)
                } ?: temperature
            val water = waterToLightMap.firstOrNull { light in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(light, it.source, it.destination, it.range)
                } ?: light
            val fertilizer = fertilizerToWaterMap.firstOrNull { water in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(water, it.source, it.destination, it.range)
                } ?: water
            val soil = soilToFertilizerMap.firstOrNull { fertilizer in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(fertilizer, it.source, it.destination, it.range)
                } ?: fertilizer
            val seed = seedToSoilMap.firstOrNull { soil in it.destination until it.destination + it.range }
                ?.let {
                    calculateDestination(soil, it.source, it.destination, it.range)
                } ?: soil
            seedRangeList.any {
                seed in it
            }
...
data class Coordinates(val destination: Long, val source: Long, val range: Long)

fun calculateDestination(input: Long, destination: Long, source: Long, range: Long): Long {
    return if (input in source until source + range)
