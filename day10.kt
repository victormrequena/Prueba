package com.victormrequena.day10

fun main() {
    val input = """L77FF.FJF|FL-7F7FF-F7F7FL.|.F-77.F--|.J7.FFLL--|7-7.7-L77F7-7.F-7F7-FF7F|77.|-JF7-FFF.L-7FFF7---|F7.|.FLF-JF-|-7---|F77FF-F-77FL77|-77.|-J.F
...
L.L--.J.L7-7JJL.|-JL-LL-7L|J-L.FJ-JLLLJ.L7J|JJF-7JJ.LJ.L--FJLLJ.J-J-JJ.L--|L-L.L-F.JLFLF-J-|-.L|---LL7JJ.L...|LL.-LF-FLFJJ7---JJJ|JL-J.L-L--"""
    ...
    val startingPosition = findStart(map)!!
    visitedPositions.add(startingPosition)

    var nextPipes = getNextPipes(map, startingPosition)
    while (nextPipes.any { it !in visitedPositions }) {
        nextPipes = nextPipes - visitedPositions
        visitedPositions.addAll(nextPipes)
        val newPipes = mutableSetOf<Pair<Int, Int>>()
        nextPipes.flatMapTo(newPipes) { pair -> getNextPipes(map, pair) }
        nextPipes = newPipes
    }
    val result = visitedPositions.size / 2
    println(result)

    val loop = visitedPositions.toSet()
    findNest(map, loop)
...
fun findStart(map: List<List<Char>>): Pair<Int, Int>? {
    val coordinates = map.mapIndexed { index, chars ->
        index to chars.indexOf(start)
    }.firstOrNull {
        it.second != -1
    }
    return coordinates
...
fun getNorth(map: List<List<Char>>, position: Pair<Int, Int>): Char? {
    val (x, y) = position
    return if (x - 1 >= 0) map[x - 1][y] else null
}

fun getSouth(map: List<List<Char>>, position: Pair<Int, Int>): Char? {
    val (x, y) = position
    return if (x + 1 <= map.lastIndex) map[x + 1][y] else null
}

fun getWest(map: List<List<Char>>, position: Pair<Int, Int>): Char? {
    val (x, y) = position
    return if (y - 1 >= 0) map[x][y - 1] else null
}

fun getEast(map: List<List<Char>>, position: Pair<Int, Int>): Char? {
    val (x, y) = position
    return if (y + 1 <= map[0].lastIndex) map[x][y + 1] else null
}

fun isNorthConnection(char: Char?): Boolean {
    return when(char) {
        SWPipe, NSPipe, SEPipe -> true
        else -> false
    }
}

fun isSouthConnection(char: Char?): Boolean {
    return when(char) {
        NWPipe, NSPipe, NEPipe -> true
        else -> false
    }
}

fun isWestConnection(char: Char?): Boolean {
    return when(char) {
        NEPipe, WEPipe, SEPipe -> true
        else -> false
    }
}

fun isEastConnection(char: Char?): Boolean {
    return when(char) {
        NWPipe, WEPipe, SWPipe -> true
        else -> false
    }
}

fun getNextPipes(map: List<List<Char>>, position: Pair<Int, Int>): Set<Pair<Int, Int>> {
    val nextPipes = mutableSetOf<Pair<Int, Int>>()
    val (x, y) = position
    when (map[x][y]) {
        start -> {
            val north = getNorth(map, position)
            if (isNorthConnection(north)) nextPipes.add(x - 1 to y)
            val south = getSouth(map, position)
            if (isSouthConnection(south)) nextPipes.add(x + 1 to y)
            val west = getWest(map, position)
            if (isWestConnection(west)) nextPipes.add(x to y - 1)
            val east = getEast(map, position)
            if (isEastConnection(east)) nextPipes.add(x to y + 1)
        }
        NSPipe -> {
            val north = getNorth(map, position)
            if (isNorthConnection(north)) nextPipes.add(x - 1 to y)
    ...
    return nextPipes.toSet()
}

fun findNest(map: List<List<Char>>, loop: Set<Pair<Int, Int>>) {
    val unnecessary = Regex("F-*7|L-*J")
    val change = Regex("\\||L-*7|F-*J")

    val loopRangesByLine = loop
        .groupBy { it.first }
        .mapValues { toRanges(it.value.map { pair -> pair.second }) }

    println(loopRangesByLine)

    val x = map
        .asSequence()
        .mapIndexed { index, chars ->
            chars to loopRangesByLine[index]
        }
        .filter { it.second != null }
        .map { it.first.joinToString(separator = "") to it.second }
        .map { pair ->
            val chars = pair.first.slice(pair.second!!.first().first..pair.second!!.last().last)
            val first = pair.second!!.first().first
            val ranges = toRanges(pair.second!!.flatMap { range -> range.map { it - first } })
            chars to ranges
        }
        .map { (chars, ranges) ->
            val rangesToText = ranges.map {
                it to chars.slice(it)
            }
                .map { pair ->
                    pair.first to unnecessary.replace(pair.second, "")
                }
                .map { pair ->
                    pair.first to change.replace(pair.second, "1")
                }
                .map { pair ->
                    pair.first to pair.second.length
                }
            chars to rangesToText
        }
        .filter { (_, ranges) ->
            ranges.any { (_, changes) ->
                changes % 2 != 0
            }
        }
        .map { (text, ranges) ->
            text to ranges.dropWhile { (_, changes) ->
                changes % 2 == 0
            }
        }
        .map { (text, ranges) ->
            text to ranges.dropLastWhile { (_, changes) ->
                changes % 2 == 0
            }
        }
        .sumOf { (_, ranges) ->
            groupRanges(ranges).sumOf { countInsideElements(it) }
        }
...
fun toRanges(indices: List<Int>): List<IntRange> {
    return indices.sorted()
        ...
        .fold(listOf()) { a, b ->
            if (a.isEmpty()) {
                return@fold listOf(b)
            }
            if (a.last().last == b.first - 1) {
                val last = a.last()
                val newLast = last.first .. b.last
                a.dropLast(1) + listOf(newLast)
            } else {
                a + listOf(b)
            }
        }
...
fun countInsideElements(ranges: List<Pair<IntRange, Int>>): Int {
    return ranges.last().first.first - ranges.first().first.last - 1 - ranges.drop(1).dropLast(1).sumOf { it.first.count() }
}

fun groupRanges(ranges: List<Pair<IntRange, Int>>): List<List<Pair<IntRange, Int>>> {
    val oddIndices = ranges.withIndex().filter { (_, value) ->
        value.second % 2 != 0
    }.map { it.index }.chunked(2)

    return oddIndices.map {
        ranges.subList(it.first(), it.last() + 1)
    }
}
